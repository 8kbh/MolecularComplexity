...
class SCScorer():
    ...
    def restore(self, weight_path=os.path.join(project_root, 'scscorer', 'models', 'full_reaxys_model_1024bool',
                                               'model.ckpt-10654.as_numpy.pickle'), FP_rad=FP_rad, FP_len=FP_len):
        self.FP_len = FP_len;
        self.FP_rad = FP_rad
...
if __name__ == '__main__':
    ...
    model = SCScorer()
    model.restore(
        os.path.join(project_root, 'models', 'full_reaxys_model_1024uint8', 'model.ckpt-10654.as_numpy.json.gz'))
    smis = ['CCCOCCC', 'CCCNc1ccccc1']
    for smi in smis:
        (smi, sco) = model.get_score_from_smi(smi)
        # print('%.4f <--- %s' % (sco, smi))
